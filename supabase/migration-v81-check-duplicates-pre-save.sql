-- v81: check_product_duplicates_candidates
--
-- Funcion para chequeo pre-save de duplicados al crear/editar un producto.
-- Devuelve candidatos rankeados por score multi-criterio (SKU, EAN, codigo
-- fabricante/proveedor, nombre normalizado, similitud fuzzy, marca, precio).
--
-- Score:
--   100+ -> casi seguro duplicado (severity "fuerte"): la UI bloquea "crear igual"
--    80+ -> "medio"
--    60+ -> "debil"  (threshold por default)
--
-- Llamada desde catalogo/page.tsx antes de tt_products.insert/update.
-- Si count > 0 abre DuplicateWarningModal con la lista.
...
BEGIN
  ...
  ranked AS (
    ...
    FROM scored s
    WHERE s.base_score >= p_min_score - 15
  ),
  top AS (
    SELECT *
    FROM ranked
